...
if __name__ == "__main__":	
    ...
    with h5py.File(f, 'r') as file:
        M = file['M'][:]
        phi = file['phi'][:]
        disorder_alls = file['disorder_all'][:]
    vals = np.nanmean(disorder_alls, axis=1)
    phi_unique = np.sort(np.unique(phi))
    M_unique = np.sort(np.unique(M))
    ...
    plt.imshow(
        ...
        aspect='auto',
        cmap='jet',
        extent=[0., np.pi, 0., np.unique(M).max()]
    )
    plt.xlabel('$\\phi$')
    plt.ylabel('M')
